    ...
    for live in [false, true] {
        ...
        let prepared = if live {
            with_live_transcript(&path, &growth, || {
                prepare_sources(&snapshot, &mut cache, 0, Watermark::NONE)
            })
        } else {
            prepare_sources(&snapshot, &mut cache, 0, Watermark::NONE)
        };
...
fn a_same_size_rewrite_since_the_inventory_counts_uncached_and_not_final() {
    ...
    let prepared = prepare_sources(&snapshot, &mut cache, 0, Watermark::NONE);
...
fn an_unreadable_transcript_counts_its_last_cached_parse_and_is_not_final() {
    ...
    let prepared = prepare_sources(&snapshot, &mut stale, 0, Watermark::NONE);
