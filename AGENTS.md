...
- `src-tauri/src/read_revision.rs`: the counter every process-wide cached read carries. The Limits screen, its menu-bar popover, the limits monitor and the notch rail share one `limits_refresh` read per provider; the Pull requests screen, the CI monitor and the notch's pull-request cell share one `github` read. Whichever gets there first fetches, and the rest must not go on showing their own older copy: a `Revision` moves on every replacement so a consumer that caches the answer (the notch's `Poll::due_from`) re-reads at once, and `announce` emits `shared-read-changed` so the queries refetch — cache-served, so no extra provider call, and silent for a revision already announced, which is what stops the loop. Route a new shared read through this rather than adding a second freshness scheme; a read that discards rather than replaces its cached value on failure (as `github` does) must not announce, or every consumer stampedes the source it was just told it could not reach.
