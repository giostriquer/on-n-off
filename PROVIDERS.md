...
| MCP | `~/.claude.json` → `mcpServers`, disabled when `disabled: true` **or** listed in `disabledMcpServers`; app patches that file. Read-only rows (`claude_mcp.rs`) add the servers that are not the user's own. Each enabled plugin's: its manifest's `mcpServers` (inline, a path under the plugin root, or a list of either) and its root `.mcp.json` (a bare server map, or one wrapped in `mcpServers`), both read, the manifest winning a name defined twice; each is `plugin:<plugin>:<server>`, `via` the plugin, with `${CLAUDE_PLUGIN_ROOT}` left unexpanded, and off when that scoped name is in `disabledMcpServers`. A disabled plugin contributes none. In the all-projects view, each local-scope server from `projects[*].mcpServers`: one row per definition (name, transport and source), on when any of its projects has it on, `via` the project's folder or how many projects have it; these are not counted as live there, and a project's own view drops them and shows its servers as project rows instead. **docs** (2026-09, Claude Code plugins reference): the manifest's three forms, `${CLAUDE_PLUGIN_ROOT}`, the scoped `plugin:<plugin>:<server>` name; which of the manifest and `.mcp.json` wins a shared name is not documented. **verified** (2026-09, real home): a plugin `.mcp.json` as a bare map, both file shapes among marketplace plugins, and local-scope servers under `projects`. **code**: `disabledMcpServers` naming a plugin server by its scoped name has not been observed. |
