...
| Subscription dates | **Code; undocumented provider data:** the login's ID token carries `chatgpt_subscription_active_until`, the end of the paid period, and `chatgpt_subscription_last_checked` under its `https://api.openai.com/auth` claims. The signed-in login's token is read from the Codex home, a saved profile's from the account vault; nothing is fetched, persisted or prompted for. The Limits card shows **Until <date>**, with the exact date, when OpenAI last confirmed it, and the caveat that the token does not say whether the plan renews, in its tooltip. A date that has passed shows nothing. Same on Windows and macOS. Until v0.18.1 a macOS helper read the date from the browser's ChatGPT session instead; it needed the browser's Keychain item, which asked again on every ad-hoc build, and could only ever see the one account the browser was signed in to; the dates it kept under `~/.on-n-off/subscriptions/` are removed once at startup. |
...
own plan names. The subscription badge to the left of the plan shows **Until <date>** from the login's ID token. Hover or
keyboard focus shows the exact device-local date and clock time, when OpenAI last confirmed it, and that
the token does not say whether the plan renews. Missing and elapsed dates have no badge. A local minute
timer drops a badge whose date passes; these badges introduce no additional requests or polling.
