...
Subscription billing refreshes reuse the per-account UI cache for 24 hours. While Limits is
visible, the current account can refresh at most once a day after a successful manual browser
import. Opening or focusing fresh cards does not reread their files. The backend persists an
attempt timestamp before an automatic import, including failures, so restarting cannot bypass
the daily cooldown. Concurrent imports are suppressed; account identity is checked before and
...
also removes eligibility for automatic imports. No browser credentials are retained by the app.
...
with auto-renewal becomes **Auto-renews**, `will_renew=false` becomes **No renewal** until its date,
and **Expired** after that date. An elapsed renewal date is **Unconfirmed**, since renewal may
have completed since the observation. Token-only future dates show **Active · cached**, without
inferring renewal. Stale billing status also carries a cached qualifier; missing dates have no
status badge. No additional request or polling is introduced for these badges.
