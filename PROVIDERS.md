...
| MCP | global `~/.cursor/mcp.json` (`mcpServers`, `command/args/env` or `url/headers`), project `.cursor/mcp.json`. App toggles by patching `disabled: true` (same parser as Antigravity). **Caveat (verified 2026-08-18):** the Cursor CLI does not read that key — `agent mcp enable/disable` keeps per-project lists in `~/.cursor/projects/<slug>/mcp-approvals.json` / `mcp-disabled.json`, and the IDE keeps its own state; whether the IDE honours `disabled` in `mcp.json` is unverified. |
